...
// Sample data - Replace this with your actual data parsing and processing logic
// This array contains objects with various properties for different classes of Alcohol
const sampleData = [
  {
    Alcohol: 1,
    "Malic Acid": 14.23,
...
// Function to calculate statistics for a given property in the sampleData
// It calculates mean, median, and mode for each class of Alcohol
const calculateStatistics = (data, property) => {
  // Create a map to store the values of the property for each class of Alcohol
  const classMap = new Map();

  // Iterate through the data and populate the classMap
  data.forEach((item) => {
    const alcoholClass = item.Alcohol;
    const value = item[property];

    if (!classMap.has(alcoholClass)) {
      classMap.set(alcoholClass, [value]);
    } else {
      classMap.get(alcoholClass).push(value);
    }
  });

  // Calculate mean, median, and mode for each class of Alcohol
  const classStats = Array.from(classMap.keys()).map((alcoholClass) => {
    const values = classMap.get(alcoholClass);
...
// Component to display the statistics in a table format
const StatTable = ({ data, property }) => {
  // State to store the calculated statistics
  const [statistics, setStatistics] = useState([]);

  // Calculate the statistics whenever the data or property changes
  useEffect(() => {
    const classStats = calculateStatistics(data, property);
    setStatistics(classStats);
  }, [data, property]);
    ...
    <table>
      <thead>
        <tr>
          ...
          {statistics.map(({ alcoholClass }) => (
            <th key={alcoholClass}>{alcoholClass}</th>
          ))}
        </tr>
      </thead>
      <tbody>
        {/* Render rows for mean, median, and mode */}
        <tr>
          <td>{property}</td>
          {statistics.map(({ mean }) => (
...
const App = () => {
  return (
    <div>
      {/* Display statistics for Flavanoids */}
      <h2>Flavanoids Statistics</h2>
      <StatTable data={sampleData} property="Flavanoids" />

      {/* Calculate Gamma property based on Ash, Hue, and Magnesium and display its statistics */}
